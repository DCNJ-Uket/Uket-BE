...
public class UniversityEventService {
    ...
    public List<ActiveOrganizationsResponse> getActiveOrganizations() {
        LocalDate now = LocalDate.now();
        List<UniversityDto> universityDtos = eventRepository.searchUniversitiesByDate(now);

        return universityDtos.stream().map(universityDto -> {
            return ActiveOrganizationsResponse.builder()
                .id(universityDto.id())
